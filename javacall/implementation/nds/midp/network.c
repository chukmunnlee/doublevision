...
//Important: Must call after timer initialization - requires timer
javacall_result javacall_network_init_start(void) {
    ...
    javacall_printf("Network initialized\n");
...
javacall_result javacall_network_finalize_start(void) {
	...
	javacall_printf("Network disconnected\n");
...
javacall_result javacall_network_gethostbyname_start(char *hostname,
	...
	//TODO: convert this to non-blocking with 3 sec retry
	...
	} while ((myhost == NULL) && (retry <= RETRY));
	if (myhost == NULL) {
           javacall_print("ghn failed, return.\n");
           return (JAVACALL_FAIL);
	} else {
           javacall_print("myhost is not null, go on.\n");
	}

	//struct in_addr *_addr = (struct in_addr *)myhost;

	if (myhost->h_length > maxLen) {
           javacall_print("too long.\n");
           return (JAVACALL_FAIL);
	}

	*pLen = myhost->h_length;
	memcpy(pAddress, myhost->h_addr_list[0], *pLen);
...
javacall_result javacall_network_gethostbyname_finish(unsigned char *pAddress,
	...
	//Should never call this, since gethostbyname will never return JAVACALL_WOULD_BLOCK
	return (JAVACALL_FAIL);
}  
