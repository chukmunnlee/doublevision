...
#define TEXTFIELD_MAX_LENGTH 256

extern volatile u8 screen_mode;
extern void clear_subscreen();
extern javacall_result javautil_unicode_utf16_to_utf8();
...
javacall_textfield_status javacall_input_textfield_launch(
    ...
    int key, buttons;
    int result = JAVACALL_TEXTFIELD_ERROR;
    int utf8len;
    unsigned char *tmpBuff = (unsigned char *)malloc(sizeof(unsigned char) * TEXTFIELD_MAX_LENGTH);

    if (tmpBuff == NULL)
        return (JAVACALL_OUT_OF_MEMORY);
    memset(tmpBuff, 0, TEXTFIELD_MAX_LENGTH);

    if (JAVACALL_OK != javautil_unicode_utf16_to_utf8(textBuffer, (javacall_int32)*textBufferLength 
            , tmpBuff, TEXTFIELD_MAX_LENGTH, &utf8len)) {
        free(tmpBuff);
        return (JAVACALL_TEXTFIELD_ERROR);
    }

    irqDisable(IRQ_VBLANK);
    //Note tried to remove the keyboard event and renable it, but does not seem to work
    //disable keyboard event - we are doing the reading
    in_keyboard = true;

    clear_subscreen();
    clear_screen();
    consoleDemoInit();
    Keyboard *keyboard = keyboardInit(NULL, 3, BgType_Text4bpp, BgSize_T_256x512, 20, 0, false, true);
    ...
    while (1) {
        ...
        iprintf("\x1b[0;0H%-256s", tmpBuff);

        scanKeys();
        buttons = keysDown();

        if (buttons & KEY_A) {
            if (javautil_unicode_utf8_to_utf16(tmpBuff, utf8len, textBuffer
                    , (javacall_int32)TEXTFIELD_MAX_LENGTH, textBufferLength) == JAVACALL_OK) {
                *caretPos = *textBufferLength;
                result = JAVACALL_TEXTFIELD_COMMIT;
            }
            goto out;
        } else if (buttons & KEY_B) {
            result = JAVACALL_TEXTFIELD_CANCEL;
            goto out;
        }
        ...
        switch (key) {
            ...
            case 8: //backspace
                ...
                break;
            default: 
                //TODO implement MIDP field constraints later
                if ((isprint(key)) && (utf8len < TEXTFIELD_MAX_LENGTH))
                    tmpBuff[utf8len++] = key;
                break;
        }
        swiWaitForVBlank();
