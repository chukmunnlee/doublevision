...
void /* OPTIONAL */ javanotify_server_socket_event(javacall_server_socket_callback_type type,
    ...
    if (type == JAVACALL_EVENT_SERVER_SOCKET_ACCEPT_COMPLETED) {
        ...
        if (operation_result == JAVACALL_OK) {
            /* Bug! Can't be compiled by gcc, need to be fixed. */
...
void javanotify_textfield_complete(javacall_textfield_status status){
    ...
    e.eventType = MIDP_JC_EVENT_TEXTFIELD;
    e.data.textFieldEvent.status = status;

    REPORT_INFO(LC_CORE,"javanotify_textfield_complete!\n");
    javacall_event_send((unsigned char*)&e, sizeof(midp_jc_event_union));
}
#endif
