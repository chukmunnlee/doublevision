...
//                                     | (event->data.multimediaEvent.playerId & 0xFFFF));
    ...
    case MIDP_JC_EVENT_TEXTFIELD:
        pNewSignal->waitingFor = TEXT_INPUT_SIGNAL;
        pNewSignal->descriptor = 0; /* FIXME: Add descriptor */
        pNewSignal->status     = event->data.textFieldEvent.status;
        pNewMidpEvent->type    = MIDP_KEY_EVENT;
        break;
#endif
